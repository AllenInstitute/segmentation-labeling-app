...
@pytest.mark.parametrize(
    "input_fps, output_fps, random_seed, strategy,expected_video",
    [
        (
            10, 2, 10, "random",
            np.array([[[3, 2], [10, 7]],
...
def test_coordinate_window_shift(coordinate_pair, window_size, video_shape,
                                 expected_pair):
    transformed_center = transformations.get_transformed_center(
        coordinate_pair, window_size, video_shape)
...
@pytest.mark.parametrize(
    "video,coord,box_size,expected",
    [(test_video, (0, 0), (3, 3),
      np.stack([np.array([[0, 1, 2],
                          [5, 6, 7],
                          [10, 11, 12]])]*2)),
     (test_video, (2, 2), (3, 3),
        np.stack([np.array([[6, 7, 8],
                            [11, 12, 13],
                            [16, 17, 18]])]*2)),
     (test_video, (-1, -1), (3, 3),
        np.stack([np.array([[0, 1, 2],
                            [5, 6, 7],
                            [10, 11, 12]])]*2)),
     (test_video, (10, 10), (3, 3),
        np.stack([np.array([[12, 13, 14],
                            [17, 18, 19],
                            [22, 23, 24]])] * 2)),
     (test_video, (10, 0), (3, 3),
        np.stack([np.array([[10, 11, 12],
                            [15, 16, 17],
                            [20, 21, 22]])] * 2)),
     (test_video, (0, 10), (3, 3),
        np.stack([np.array([[2, 3, 4],
                            [7, 8, 9],
                            [12, 13, 14]])] * 2)),
     (test_video, (-1, 2), (3, 3),
...
def test_get_centered_coordinate_box_video(video, coord, box_size, expected):
    ...
    Test Cases:
        ...
        4) Subset center out of bounds on the lower right corner by
            double video size
        5) Subset center subset out of bounds on the lower left corner
            by double video height
        6) Subset center out of bound on the upper right corner by
            double video width
        7) Subset center centered in the x dimension and negative in
            the y coordinate space
        8) Subset center centered in the y dimension and negative in
    ...
    """
    result = transformations.get_centered_coordinate_box_video(
        coord, box_size, video)
